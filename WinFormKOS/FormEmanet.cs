...
namespace WinFormKOS
{
    public partial class FormEmanet : Form
    {
        ...
        void kitaplarLoad()
        {
            dgKitaplar.DataSource = IDataBase.DataToDataTable("select * from kitaplar where aktif=1 and durum=1 and kitapAdi+' '+yazarAdi like @search", new SqlParameter("@search", SqlDbType.VarChar) { Value = string.Format("%{0}%", txtFiltreleKitap.Text) });
        ...
        void emanetEt()
        {
            ...
            {
                MessageBox.Show("Kitap ya da okuyucu seçmediniz!");
                return;
            }
            if (getEmanetId()>0)
            {
        ...
        void dusumYap()
        {
            ...
            IDataBase.executeNonQuery("update kitaplar set durum=1 where id=@kitapId " + "update emanetler set emanetIslemTarihi=@emanetIslemTarihi, durum=1 where kitapId=@kitapId", parameters);
            getOkuyucuProfil();
            kitaplarLoad();
        }
        ...
        private void btnSureUzat_Click(object sender, EventArgs e)
        {
            if (cezaTL>0)
            {
                DialogResult dialogResult = MessageBox.Show("Okuyucunun " + string.Format("{0:c}", cezaTL) + " var. Teslim alındı mı?", "Gecikme Bedeli", MessageBoxButtons.YesNo);
