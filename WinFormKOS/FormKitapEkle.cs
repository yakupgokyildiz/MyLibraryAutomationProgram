...
namespace WinFormKOS
{
    public partial class FormKitapEkle : Form
    {
        ...
        void comboBoxFill()
        {
            ...
            {
                cbbTuru.Items.Add(row["adi"].ToString());
            }
        ...
        void kitapLoad()
        {
            dg.DataSource = IDataBase.DataToDataTable("select * from kitaplar where aktif=1 and kitapAdi+' '+yazarAdi like @search",
        ...
        void kitapEkle()
        {
            ...
            parameters.Add(new SqlParameter("@tur", SqlDbType.VarChar) { Value = cbbTuru.Text});
        ...
        {
            if (string.IsNullOrEmpty(txtKitapAdi.Text)|| string.IsNullOrEmpty(cbbYazarAdi.Text)|| string.IsNullOrEmpty(cbbTuru.Text))
            {
                MessageBox.Show("Kitap adı, yazar adı ve tür boş geçilemez!");
                return;
            }
            if (kitapId>0)
            {
